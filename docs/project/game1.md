...
DaoDomainSystem （御主空间系统 --支线玩法），
ShopSystem  （商店系统 --商店），
GachaSystem （抽奖系统 --概率相关），
TrainSystem （训练系统 --角色训练相关），
FightSystem （战斗系统 --角色战斗），
BagSystem （背包系统 --物品增删改查），

- 数据驱动设计（excel编辑，然后使用js脚本 ,转化为可用ts文件）
...
项目的起点在于GlobalSystem,这里进行全局初始化，首先初始化存储服务，依次调用各个系统，各个系统内部首先使用DomainFactory构建Domain，然后构建系统各部分。
